...
import { DndContext, DndProvider, DragSourceMonitor, useDrag } from 'react-dnd'

import { changeDateMonth } from 'src/components/DatePicker/Date'

import { getCalendarRows } from './Calendar'
...
export const DatePickerCalendar: React.FC<IDatePickerCalendarProps> = ({
  ...
  const [{ opacity }, dragRef] = useDrag(
    () => ({
      type: 'CARD',
      item: { type: 'CARD', id: 'card' + Math.random() },
      collect: (monitor: DragSourceMonitor<string, unknown>) => ({
        opacity: monitor.isDragging() ? 0.5 : 1,
      }),
    }),
    []
  )
  const isDragging = opacity === 0.5
  return (
    <>
      <div className="flex h-full flex-col">
        <header className="relative z-20 flex flex-none items-center justify-between border-b border-gray-200 py-4 px-6">
          <h1 className="text-lg font-semibold text-gray-900">
            <time dateTime="2022-01">{shownDate.format('MMMM YYYY')}</time>
          </h1>
